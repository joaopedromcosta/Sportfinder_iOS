...
class DetalhesLocalViewController: UIViewController {
    ...
    //
    var detLocal = EntityLocal(string: "a")
    // MARK: Labels
    ...
    func getLocalidade(){
        ...
        let urlJsonString = "https://sportfinderapi.000webhostapp.com/slim/api/getDadosLocalidade/"+"30"
        print(urlJsonString)
        ...
        URLSession.shared.dataTask(with: url) { (data, response, error) in
            guard let data = data else {
             ...
             DispatchQueue.main.async {
                do {
                    let response = try JSONDecoder().decode([EntityLocal].self, from: data)
                    for d in response {
                        print(d.nome)
                        self.detLocal = d
                        // Labels
